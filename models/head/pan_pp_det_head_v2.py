...
class Self_Attn(nn.Module):
    ...
    def forward(self,x):
        ...
        _, _, ori_w, ori_h = x.size()

        res = x
        
        x = F.adaptive_avg_pool2d(x, (ori_w // 2, ori_h // 2))
        # x = F.adaptive_avg_pool2d(x, (80,80))
        ...
        out = F.upsample(out, (ori_w, ori_h), mode='bilinear')

        out = self.gamma*out + res
        return out


class PAN_PP_DetHead_v2(nn.Module):
    def __init__(self,
                 in_channels,
                 hidden_dim,
                 num_classes,
                 ...
                 loss_emb,
                 use_coordconv=False):
        super(PAN_PP_DetHead_v2, self).__init__()
        ...
        # self.conv_res = nn.Conv2d(in_channels, 
        #                           num_classes, 
        #                           kernel_size=1,
        #                           stride=1,
        #                           padding=0)
        ...
        # self.self_attn = Self_Attn(in_channels)
    ...
    def forward(self, f):
        # f = self.self_attn(f) # self attention
        out = self.conv1(f)
        out = self.relu1(self.bn1(out))
        out = self.conv2(out)

        # binary = self.binarize(f)
        # thresh = self.thresh(f)
        # thresh_binary = self.step_function(binary, thresh)
        
        # out = out + self.conv_res(f) # res
    ...
    def thresh_loss(self, pred, gt, mask):
        mask_sum = mask.sum()
        loss = (torch.abs(pred[:, 0] - gt) * mask).sum() / mask_sum
        return loss

    def loss(self, out, gt_texts, gt_kernels, training_masks, gt_instances,
             gt_bboxes, thresh_map, thresh_mask):
        texts = out[:, 0, :, :]
        kernels = out[:, 1:2, :, :]
        embs = out[:, 2:6, :, :]

        # loss_thresh = self.thresh_loss(thresh, thresh_map, thresh_mask)
        ...
        # losses.update(dict(loss_thresh=loss_thresh))
