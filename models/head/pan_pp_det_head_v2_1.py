...
class PAN_PP_DetHead_v2_1(nn.Module):
    def __init__(self,
                 in_channels,
                 hidden_dim,
                 num_classes,
                 ...
                 loss_emb,
                 use_coordconv=False):
        super(PAN_PP_DetHead_v2_1, self).__init__()
        ...
        self.attn = attn(in_channels // 3)

    def forward(self, f):

        part1 = f[:, :self.csp_ch, :, :]
        part2 = f[:, self.csp_ch:self.csp_ch*2, :, :]
        part3 = f[:, self.csp_ch*2:self.csp_ch*3, :, :]

        out = self.attn(part1, part2, part3)
