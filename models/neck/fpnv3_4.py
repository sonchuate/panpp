...
class Conv_block(nn.Module):
    def __init__(self, in_channels, out_channels, stride):
        ...
        # self.dwconv = nn.Conv2d(in_channels // 2, in_channels // 2, kernel_size=3, stride=stride, padding=1, groups=in_channels // 2, bias=False)
        self.smooth_layer1 = Conv_BN_ReLU(in_channels // 3, out_channels // 3)
        self.smooth_layer2 = Conv_BN_ReLU(in_channels // 3, out_channels // 3)
        self.smooth_layer3 = Conv_BN_ReLU(in_channels - 2 * (in_channels // 3), out_channels - 2 * (out_channels // 3))
        # self.conv = nn.Conv2d(in_channels // 2, out_channels // 2, kernel_size=1, stride=1, padding=0, bias=False)
        # self.bn = nn.BatchNorm2d(out_channels // 2)
        # self.relu = nn.ReLU(inplace=True)

        self.csp_ch = int(in_channels // 3)


    def forward(self, x):

        part1 = x[:, :self.csp_ch, :, :]
        part2 = x[:, self.csp_ch:self.csp_ch*2, :, :]
        part3 = x[:, self.csp_ch*2:, :, :]

        part1 = self.smooth_layer1(part1)
        part2 = self.smooth_layer2(part2)
        part3 = self.smooth_layer3(part3)


        out = torch.cat((part1, part2, part3), 1)
    ...
    #     part1 = self.dwconv(part1)
    #     part1 = self.smooth_layer(part1)

    #     part2 = self.conv(part2)
    #     part2 = self.bn(part2)
    #     part2 = self.relu(part2)

    #     x = torch.cat((part1, part2), 1)
...
class FPN_v3_4(nn.Module):

    def __init__(self, in_channels, out_channels):
        super(FPN_v3_4, self).__init__()
